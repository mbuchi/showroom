...
export const RELEASES: Release[] = [
  {
    ...
    summary: 'showroom moves to @aireon/shared v1.210.0, whose error reporting is more accurate: page reloads, offline moments and deliberately aborted requests no longer file themselves as faults.',
    items: [
      {
        kind: 'improved' as ChangeKind,
        icon: Zap,
        text: 'Updated the shared Aireon foundation from v1.209.0 to v1.210.0. Error reporting got more accurate: a best-effort marker on a fetch and a beforeCapture veto let a request declare up front that a failure is expected, so leaving or reloading a page, a brief offline moment, or a request the app itself cancels no longer arrives in the bug tracker as an application fault. Real failures are still reported exactly as before. The reports, the map and the published views are unchanged.',
        prs: [],
      },
    ],
  },
  ...
  },
  {
    ...
    summary: 'The zone shown for a parcel is now the harmonized federal category, the same label every Aireon app prints, instead of the municipal wording.',
    items: [
      {
        kind: 'improved' as ChangeKind,
        icon: MapPin,
        text: 'The zone chip in the Reporter parcel strip, the zone fact on the PDF report and the zone line under the publish prefill now show the harmonized federal category ("Wohnzonen", "Mischzonen", "Zentrumszonen") rather than the municipal designation ("Wohnzone, Bauklasse 4"). It is one label per parcel and it reads the same in every Aireon app. Parcels that have no harmonized category yet (all of the canton of Zürich as of August 2026) keep showing their municipal designation, so nothing goes blank.',
        prs: [],
      },
      {
        kind: 'fixed' as ChangeKind,
        icon: Bot,
        text: 'Claire now actually receives the parcel zone in her context. It was handed over under keys her context builder does not read, so questions about the zone were answered without it.',
        prs: [],
      },
      {
        kind: 'improved' as ChangeKind,
        icon: Package,
        text: 'Updated @aireon/shared to v1.173.1, which carries the suite-wide zone rule (harmonized category first, municipal fallback, legal cross-references and canton codes never shown as a zone). Cached parcel facts from before this release are refreshed on next lookup so the old wording does not linger.',
        prs: [],
      },
    ],
  },
    ...
    items: [
      {
        kind: 'fixed' as ChangeKind,
        icon: Smartphone,
        text: 'Stopped iOS Safari from auto-zooming the page when the address search is focused on phones, which left the page stuck wider than the screen. Search text is now 16px on phones and the page scale stays at 100%.',
        prs: [],
      },
    ],
  },
    ...
    items: [
      {
        ...
        icon: Sparkles,
        text:
          'When you opened the app signed out, the account button in the navbar rendered as a high-contrast dark circle that clashed with the light theme. It now uses the same neutral scheme as every other map control: light grey in light mode, slate in dark mode.',
        prs: [],
      },
    ],
  },
